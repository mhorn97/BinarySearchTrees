...
public class BinarySearchTree<T extends Comparable<? super T>> {
	...
	/*
	 * public boolean add(T element) { // edge case -- is the tree empty? if (root
	 * == null) { root = new Node(element); size++; return true; }
	 * 
	 * // walk down the tree to the insertion point Node current = root; Node prev =
	 * root; int comp = 0; while (current != null) { // compare the keys comp =
	 * element.compareTo(current.element);
	 * 
	 * // if comp < 0, go left if (comp < 0) { prev = current; current =
	 * current.left; } else if (comp > 0) { // if comp > 0, go right prev = current;
	 * current = current.right; } else { // duplicate return false; } }
	 * 
	 * // at a leaf's null child if (comp < 0) { prev.left = new Node(element);
	 * size++; } else { prev.right = new Node(element); size++; }
	 * 
	 * return true; }
	 */
